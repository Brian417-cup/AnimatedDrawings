...
class AnimatedDrawingsJoint(Joint):
    ...
    def __init__(self, name: str, x: float, y: float):
        ...
        # 1. x,y is all in float type and normalized in [0,1]
        # the +y is up rather than down, therefore, it should be y = 1-y
...
class AnimatedDrawingRig(Transform):
    ...
    def __init__(self, char_cfg: CharacterConfig):
        ...
        # set up buffer for visualizing vertices (This shape I don't understand currently)
    ...
    def _set_global_orientations(self, joint: AnimatedDrawingsJoint, bvh_orientations: Dict[str, float]) -> None:
        '''
        Please attention the bone definition here is (start_point -> end_point ), and the bone is always same with end_point
        And if the bone is rotated, the rotation should be started used from parent joint in chain.
        '''
        if joint.name in bvh_orientations.keys():
            # this is to caculate global rotation compared with init pose rotation
            ...
            # caculate relative rotation between parent and child
            if hasattr(parent, 'current_theta'):
                theta = theta - parent.current_theta

            # here, due to rotation in 2D, so the rotation around +z axis
            rotation_q = Quaternions.from_angle_axis(np.array([theta]), axes=Vectors([0.0, 0.0, 1.0]))
            # parent set rotation and then recursive to the children in chain
...
class AnimatedDrawing(Transform, TimeManager):
    ...
    def __init__(self, char_cfg: CharacterConfig, retarget_cfg: RetargetConfig, motion_cfg: MotionConfig):
        ...
        self.indices: npt.NDArray[np.int32] = np.stack(
            self.mesh['triangles']).flatten()  # order in which to render triangles

        # regarget is from bvh file to the sketch in blank
    ...
    def _modify_retargeting_cfg_for_character(self):
        ...
        for position_test, target_joint_name, joint1_name, joint2_name in self.retarget_cfg.char_runtime_checks:
            if position_test == 'above':
                ...
                # 综合起来，这段代码似乎是用于检查目标向量是否位于 x 轴下方，如果是，则会删除一个关节映射
    ...
    def _initialize_retargeter_bvh(self, motion_cfg: MotionConfig, retarget_cfg: RetargetConfig):
        """ Initializes the retargeter used to drive the animated character.
            motion_cfg: define detailed bvh file.
            retarget_cfg: define body part group and mapping relationship.
        """
        ...
        # valid whether bvh file has correct format and mapping relationship
        motion_cfg.validate_bvh(bvh_joint_names)
        retarget_cfg.validate_char_and_bvh_joint_names(char_joint_names, bvh_joint_names)

        # a shorter alias
        # This is to determine which body part in bvh will be used for offset
        # And in the body part, there may has several small part that have mapping from bvh to character
        ...
        for char_joint_name, (
                bvh_prox_joint_name, bvh_dist_joint_name) in self.retarget_cfg.char_joint_bvh_joints_mapping.items():
    ...
    def update(self):
        ...
        # set local orientation for every joint
        self.rig.set_global_orientations(frame_orientations)

        # using new joint positions, calculate new mesh vertex xy positions
        # this control points for charcater 2D start pose
        # here please attention: when solving arap, the origin point must be in root joint position
    ...
    def _set_draw_indices(self, joint_depths: Dict[str, float]):
        ...
        # [(group_idx 1,depth 1),(group_idx 2,depth 2),...,(group_idx n,depth n)]
        ...
        for idx, bodypart_group_dict in enumerate(self.retarget_cfg.char_bodypart_groups):
            bodypart_depth: np.float32 = np.mean(
                [joint_depths[joint_name] for joint_name in bodypart_group_dict['bvh_depth_drivers']])
        ...
        # details about variable of  "indicies"
        # the total len(indicies)== config in bvh binded
        # [
        # (triangle_idx11,triangle_idx12,...,triangle_idx1n),
        # (triangle_idx21,triangle_idx22,...,triangle_idx2n),
        # ......
        # (triangle_idxm1,triangle_idxm2,...,triangle_idxmn)
        # ]
    ...
    def _initialize_joint_to_triangles_dict(self) -> None:  # noqa: C901
        ...
        # Please attention: Here, the joint we need to normalized into [0,1] and the +y is up, +x is right
        ...
        for _, joint in joints_d.items():
            ...
            # 这个list.extend方法相当于一次性添加多个元素到列表中, heap.extend( [ seq_item ])
        ...
        # This is a Dict type, store :
        # Here, the dist is the triangle centriod to bone(end point of a bone)
        #  {
        #       joint_name1: [ ([3 triangle1 index v10,v11,v12]==shape: [3], dist1),...,([3 trianglen index  vn0,vn1,vn2]==shape: [3], distn) ]
        #       ...
        #       joint_namem: [ ([3 triangle1 index v10,v11,v12]==shape: [3], dist1),...,([3 trianglen index  vn0,vn1,vn2]==shape: [3], distn) ]
        #  }
        ...
        for tri_v_idx in self.mesh['triangles']:
            ...
            joint_to_tri_v_idx_and_dist[joint_name_to_idx[tri_centroid_closest_joint_idx]].append(
                (tri_v_idx, dist_from_tri_centroid_to_bone))

        # Here, for each joint, there are Vi triangle cnt, each triangle consits of 3 indicies,
        # and we flatten their corresponding indicies into a [3*Vi] shape
        # Dict
        # {
        #   joint_name1(str): [3*V1],
        #   ...
        #   joint_namen(str): [3*Vn],
        #  }
    ...
    def _load_mask(self) -> npt.NDArray[np.uint8]:
        ...
        # don't understand (This problem has been solved by following answer:
        # https://github.com/facebookresearch/AnimatedDrawings/issues/220)
    ...
    def _load_txtr(self) -> npt.NDArray[np.uint8]:
        ...
        try:
            _txtr: npt.NDArray[np.uint8] = cv2.imread(str(txtr_p),
                                                      cv2.IMREAD_IGNORE_ORIENTATION | cv2.IMREAD_UNCHANGED).astype(
                np.uint8)
    ...
    def _initialize_opengl_resources(self) -> None:
        ...
        # 1. create a texture id and load corresponding texture resource for it
        ...
        # introduction about buffer can be obtained from https://zhuanlan.zhihu.com/p/109107871
        # universal buffer data use can be as following: (for some common buffers)
        #
        # buffer_id = glGenBuffers(buffer_cnt)
        # glBindBuffer( BUFFER_TYPE,buffer_id )
        # glBufferData( BUFFER_TYPE,data,data_type(static or dynamic) )
        #
        # here we mainly use two type buffer:
        # GL_ARRAY_BUFFER (store data)
        # and
        # GL_ELEMENT_ARRAY_BUFFER (store indicies)

        # And VAO is a sepcial buffer type, therefore, if we use this type buffer, we should use it with common VBO
        # and even EBO ,they should in a same location id index in OpenGL like following:
        self.vao = GL.glGenVertexArrays(1)
        self.vbo = GL.glGenBuffers(1)
        ...
        # buffer element index data ( this is static, due to indicies won't be modified )
        GL.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER, self.ebo)
        GL.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER,
                        self.indices, GL.GL_STATIC_DRAW)

        # here has a important function in OpenGL with data between cpu and gpu(vertex shader)
        # 1.generate buffer
        # GL.glGenBuffers(buffer_cnt)
        #
        # 2. decide buffer type
        # GL.glBindBuffer(GL_ARRAY_BUFFER or GL_ELEMENT_ARRAY_BUFFER or other buffer type, vbo)
        #
        # 3. fulfill buffer data
        # GL.glBufferData(GL_ARRAY_BUFFER or GL_ELEMENT_ARRAY_BUFFER or other buffer type, data, GL.GL_DYNAMIC_DRAW or GL.GL_STATIC_DRAW)
        #
        # 4. bind buffer location in corresponding vertex shader by index
        # GL.glVertexAttribPointer (
        #   index gpu location in vertex shader ,
        #   ----------------------------------------
        #      (such as "layout(location = 1)" defined in vertex shader , and the parameter value here should be settled 1)
        #      or
        #      use OpenGL API glBindAttribLocation() here:
        #
        #         glBindAttribLocation(program, 0, "position");
        #         glBindAttribLocation(program, 1, "color");
        #      two sentence is bind to vertex shaer variable:
        #         in vec3 position;
        #         in vec4 color;
        #   ----------------------------------------
        #   size (each element dimension),
        #   data_type,
        #   normalized? ,
        #   stride -- means every element in array contains how many bytes,
        #             here please attention float is x4 byte and double is x8 byte,
        #             if a element contains other contents exclude current data, the total location should also be considered
        #   pointer address -- means current start pointer address we will used(excluded rebundant data)
        #   )
        #
        # 5. let vertex shader can visit data sent data
        # GL.glEnableVertexAttribArray(index location defined in vertex shader)
    ...
    def _draw(self, **kwargs):
        '''
        Each sketch bind to a corresponding AnimatedDrawing class.
        Therefore, all should be called this function when animation rendered.
        '''

        # If first initialized, it will settled to open corresponding buffer setting for vbo,vao and ebo
        if not self._is_opengl_initialized:
            self._initialize_opengl_resources()
        ...
        # Other data need send and render
        # first, we need use glBindVertexArray
        GL.glBindVertexArray(self.vao)
        ...
        if kwargs['viewer_cfg'].draw_ad_txtr:
            ...
            # Set model transform matrix in vertex stage
            model_loc = GL.glGetUniformLocation(kwargs['shader_ids']['texture_shader'], "model")
            GL.glUniformMatrix4fv(model_loc, 1, GL.GL_FALSE, self._world_transform.T)

            # glDrawElements is used for shader render, here is its introduction:
            # glDrawElements(
            #  mode,
            #  count,
            # each element type,
            # element array init offset
            # )
            #
            # There is also a stronger function called glDrawElementsInstanced ,
            # this can let gpu draw more than one element at same time in a vertex shader stage.
            # It can be found from https://www.youtube.com/watch?v=dMVhGJkALW0

            # This is a key function to send texture data into vertex shader
            GL.glDrawElements(GL.GL_TRIANGLES, self.indices.shape[0], GL.GL_UNSIGNED_INT, None)

            GL.glEnable(GL.GL_DEPTH_TEST)

        # Render color shader (Not used currently)
        ...
        # Render color shader for mesh line
