...
class Retargeter():
    ...
    def _compute_normalized_joint_positions_and_fwd_vectors(self) -> None:
        ...
        Computes fwd vector for bvh skeleton at each frame. (That means in dynamic way.)
        ...
        angle %= 2*np.pi
        # 这步大概猜测是为了保证接下来的math.sin和math.cos传入的参数是正常的范围??
    ...
    def _determine_projection_plane_normal(self, group_name: str, joint_names: List[str], projection_method: str) -> npt.NDArray[np.float32]:
        ...
        # [T,V*C=3]
        # 这个joint_mask存储的是需要改变的每个通道对应的值，如果为False表示某个点对应的通道不需要修改，否则需要修改
    ...
    def compute_orientations(self, bvh_prox_joint_name: str, bvh_dist_joint_name: str, char_joint_name: str) -> None:
        ...
        # compute the bone vector ( parent_joint - child_joint)
        ...
        # project bone onto 2D plane
        # following step should be have same local coordinate with offset caculation
        # Therefore, following code is to transfer local 2D cooridnate system from global 3D coordinate system
        # 1. Transfer from global to local cooridnate system
        if np.array_equal(projection_plane_normal, x_axis):
