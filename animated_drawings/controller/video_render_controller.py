...
class VideoRenderController(Controller):
    ...
    def __init__(self, cfg: ControllerConfig, scene: Scene, view: View) -> None:
        ...
        # fps is settled to the first character's animation when they initilization
    ...
    def _set_frames_left_to_render_and_delta_t(self) -> None:
        ...
        # Here, for every child character, they may have different animation time
    ...
    def _finish_run_loop_iteration(self) -> None:
        # get pixel values from the default frame buffer, send them to the video writer
        GL.glBindFramebuffer(GL.GL_READ_FRAMEBUFFER, 0)
        # After this function, data will be saved in self.frame_data
        # In the frame buffer, the init point (0,0) is under left bottom,
        # here we change [H_opengl format,W,C] -> [H,W,C] OpenCV
...
class MP4Writer(VideoWriter):
    ...
    def __init__(self, controller: VideoRenderController) -> None:
        ...
        self.video_writer = cv2.VideoWriter(str(output_p), fourcc, frame_rate,
