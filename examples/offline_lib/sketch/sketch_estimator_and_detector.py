...
class SketchDetectorAndEstimator():
    ...
    def _write_result(self, img_path: str, out_dir: str, bbox: List[int] = None,
        ...
        joint_img_path = osp.join(output_base_dir, 'joint_image.' + img_suffix)
        img_joint_data = np.copy(img_data)
        for node_name, point in joint_name_to_xy_dict.items():
            print(f'>> Current node name is {node_name}')
            x, y = point
            cv2.circle(img_joint_data, (x, y), 4, (0, 0, 255), thickness=-1, lineType=cv2.FILLED)
        cv2.imwrite(joint_img_path, img_joint_data)
