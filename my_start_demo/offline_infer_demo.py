...
import os.path as osp
from deployment.sketch_estimator_and_detector import SketchDetectorAndEstimator
from tool.config import convert_easydict_to_dict
...
def combine_cfg_and_args():
    ...
    main_config_file = osp.join(r'offline_config/mvc/video_base.yaml')
    ...
    def get_args():
        ...
        parser.add_argument('--src_motion', type=str, default='offline_config/bvh/clapping_hand_smartbody.bvh',
...
def reload_config_files(main_cfg=None):
    from tool.config import get_config
    if main_cfg is None:
        main_cfg = get_config('offline_config/mvc/video_base.yaml')
...
def motion_info_acquire(main_cfg, motion_cfg, input_file_path, pose3d_cfg=None):
    ...
    if motion_file_suffix in VIDEO_SUFFIX_LIST:
        ...
        def _check_and_add_absolute_path(cfg):
            ...
            if not osp.isabs(pose3d_cfg_path):
                pose3d_cfg_path = osp.join(osp.abspath(osp.dirname(__file__)), 'pose3d', pose3d_cfg_path)

            if not osp.isabs(pose3d_model_path):
                pose3d_model_path = osp.join(osp.abspath(osp.dirname(__file__)), 'pose3d', pose3d_model_path)

            cfg.detector_3d.model_config_path = pose3d_cfg_path
            cfg.detector_3d.checkpoint_path = pose3d_model_path

        from tool.config import get_config
        from pose3d.infer_pose3d_with_bvh import get_vid_info
        from pose3d.infer_pose3d_with_bvh import main as video_infer_main
    ...
    from pose3d.tool.bvh.deparser.bvh import BVH
